...
const ProductPurchaseSection = () => {
  ...
  return <section className="section-major bg-background" id="buy">
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-start">
            ...
            <div className="relative lg:sticky lg:top-24">
              ...
              background: "linear-gradient(145deg, hsl(200 70% 97%) 0%, hsl(210 30% 96%) 100%)"
                ...
                <img src={productImage} alt="TerraFreeze Pain Relief Cream" className="w-full max-w-xs mx-auto object-contain" />
              </div>
            ...
            <div>
              ...
              <div className="space-y-2 mb-6">
                {packages.map(pkg => <button key={pkg.id} onClick={() => setSelectedPackage(pkg.id)} className={`w-full p-3.5 rounded-lg border-2 transition-all duration-200 text-left ${selectedPackage === pkg.id ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedPackage === pkg.id ? "border-primary bg-primary" : "border-border"}`}>
                        ...
                        <div>
                          ...
                          {pkg.popular && <span className="ml-2 px-2 py-0.5 bg-amber-100 text-amber-700 text-xs font-medium rounded">
                              ⭐ Most Popular
                            </span>}
                        </div>
                      </div>
